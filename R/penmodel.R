...
penmodel <- function(formula, cluster="famID", gvar="mgene", parms, cuts=NULL, data, design="pop", base.dist="Weibull", frailty.dist="none", agemin=NULL, robust=FALSE){
  ...
  # revised
  if(length(x.names)==1) i.missing <- is.na(data[, x.names]) # indicates if any x variable is missing
  else i.missing <- apply(data[, x.names], 1, anyNA) # indicates if any x variable is missing
  ...
  if(frailty.dist=="none"){
    ...
    if(!design %in% c("pop", "pop+", "noasc"))  stop("design should be one of pop, pop+ or noasc.")
