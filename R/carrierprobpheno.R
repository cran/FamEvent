...
carrierprobpheno <- function(method="data", fit=NULL, data, mode="dominant", q=0.02)
{
  ...
  if(is.null(data$gender)) stop("gender is missing.")
  ...
  if(method=="data"){
    ...
    if(is.null(majorgene)){
      for(g in unique(data$relation)){
        for(s in c(0,1)){
          for(d in c(0,1)){
            carrp[is.na(data$mgene) & data$relation == g & data$gender == s & data$status == d] <- mean(data$mgene[!is.na(data$mgene) & data$relation == g & data$gender == s & data$status == d], na.rm=T)
          }
        }
      }
    }
    ...
    xbeta0 <- c(X0%*%theta[(nbase+1):(nbase+nX)])
    xbeta1 <- c(X1%*%theta[(nbase+1):(nbase+nX)])
