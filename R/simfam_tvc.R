...
if(!is.element(tvc.type, c("PE", "CO"))) stop("Unrecognized tvc.type; tvc.type should be either PE or CO.") 
if(tvc.type == "PE" & length(tvc.parms)!=1) warning("The first elememnt of tvc.parms is used.")
if(tvc.type == "CO" & length(tvc.parms)!=3) warning("tvc.parms should be a vector of length 3.")
  
if(add.tvc){
  if(is.null(tvc.range)) tvc.range <- c(agemin, agemax)
  else if(tvc.range[1] < agemin) tvc.range[1] <- agemin
}
...
dat <- data.frame(familyDesign_tvc(n=N.fam, affectnum=affectnum, m.carrier=m.carrier, variation=variation,
...
if(hr==0) dat$weight <- 1 #one stage sampling
  ...
  n.hr <- sum(datp$naff>2) # number of high risk families in the simulated families
	...
	dat2 <- data.frame(familyDesign_tvc(n=n.more, affectnum=3, m.carrier=m.carrier, variation=variation,
			interaction=interaction, add.x = add.x, x.dist = x.dist, x.parms = x.parms, depend=depend, 
			add.tvc = add.tvc, tvc.type = tvc.type, tvc.range=tvc.range, tvc.parms = tvc.parms,
			base.dist=base.dist, frailty.dist=frailty.dist,
...
class(dat)<-c("simfam_tvc","data.frame")
