...
carrierprobgeno <-function(method="data", data, mode="dominant", q=0.02){
  ...
  if(is.null(gender)) stop("gender is missing.")

  if(method=="data"){
    if(is.null(majorgene)){
    for(g in unique(data$relation)){
      for(s in c(0,1)){ #gender
        carrp[is.na(data$mgene) & data$relation == g & data$gender == s] <- mean(data$mgene[!is.na(data$mgene) & data$relation == g & data$gender == s], na.rm=T)
      }
    }
   }     
  else{
